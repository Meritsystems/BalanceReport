...
def execute(filters=None):
	...
	in_item_count = 0
	out_item_count = 0
	consolidation_in_item_count = 0
	consolidation_out_item_count = 0
	consolidation_opening_qty = 0
	whse_count = 0
	in_whse_count = 0
	out_whse_count = 0
	alloc_unalloc = ""
	...
	for rows in data:
		...
		print "vehicle_status-------", rows[5]
		print "booking_reference_number-------", rows[6]
		
		if len(serialid_list) == 0:
			if rows[6] is not None and rows[5] is not None and rows[4] is not None:
				alloc_whse_count = alloc_whse_count + 1
				serialid_list.append(rows[4])
			else:
				if rows[4] is not None:
					unalloc_whse_count = unalloc_whse_count + 1
					serialid_list.append(rows[4])
		else:
			if rows[4] not in serialid_list:
				if rows[6] is not None and rows[5] is not None and rows[4] is not None:
					alloc_whse_count = alloc_whse_count + 1
					serialid_list.append(rows[4])
				else:
		...
		if total_count == 0:
			...
			if opening_qty is not None:
				consolidation_opening_qty = consolidation_opening_qty + opening_qty
			...
			qty_prev = rows[7]
			"""
			if rows[6] is not None and vehstatus_prev is not None or vehstatus_prev is not "":
				alloc_whse_count = alloc_whse_count + 1
			else:
				unalloc_whse_count = unalloc_whse_count + 1
			serialid_list.append(serial_prev)
			"""

			whse_count = whse_count + 1
		
			#if rows[6] and rows[5] == "Allocated but not Delivered":
			if rows[6] is not None and vehstatus_prev is not None and vehstatus_prev is not "":
			...
			if item_prev == item_work:
				#if rows[6] and rows[5] == "Allocated but not Delivered":
				if rows[6] is not None and vehstatus_work is not None and vehstatus_work is not "":
					alloc_unalloc = "Allocated"
				else:
					alloc_unalloc = "Unallocated"
				...
				if opening_qty is None:
					opening_qty = 0
				if in_item_count is None:
					in_item_count = 0
				if out_item_count is None:
					out_item_count = 0
				...
				if brn_work is not "" and vehstatus_work is not None and vehstatus_work is not "":
					alloc_unalloc = "Allocated"
				else:
					alloc_unalloc = "Unallocated"
	...
	summ_data.append(["", "", "", "", "", "", "", "", "", "", ""])
	summ_data.append(["", "", "", "", "", "", "", "", "", "", ""])
	summ_data.append([whse_work, "Allocated", (alloc_whse_count), "", "Unallocated", unalloc_whse_count, "", consolidation_opening_qty, 		consolidation_in_item_count, consolidation_out_item_count, (consolidation_opening_qty + consolidation_in_item_count - 			  consolidation_out_item_count)])
	if alloc_whse_count:
		alloc_whse_count = 0
	if unalloc_whse_count:
		unalloc_whse_count = 0
