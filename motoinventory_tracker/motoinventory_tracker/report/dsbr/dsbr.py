...
def execute(filters=None):
	...
	for opening_data in details:
		...
		if opening_data.posting_date < from_date:
			...
			'''
			#if opening_data.serial_no is not None:
			print "----------qty_diff------------", qty_diff
			print "----------opening_qty------------", opening_qty
			print "----------serial_no------------", opening_data.serial_no
			
			if opening_qty:
				if opening_data.serial_no:
					print "----------voucher_no------------", opening_data.voucher_no
					print "opening serial_id------------", opening_data.serial_no,  opening_data.warehouse
			'''
		bal_qty += qty_diff
	print "opening_qty-------", opening_qty
	...
	for data in stock_details:
		...
		if data['serial_no'] is not None:
			...
			if booking_reference_number is not None and booking_reference_number is not "":
				customer_name = get_customer_name_from_so(booking_reference_number)
				inward_outward_status = "Allotted" + " (" + booking_reference_number + "/"+customer_name+")"
	...
	print "unallocation_count-----", unallocation_count
	total = int(allocation_count) + int(unallocation_count)
	print "total-----", total
	summ_data.append(["Opening Stock", "", "Total:",int(opening_qty)])
	summ_data.append(["", "","", ""])
	summ_data.append(["Closing Stock", "Allotted", int(allocation_count), "Free Stock", unallocation_count, "Total:", (total)])
	summ_data.append(["", "","", ""])
...
def get_customer_name_from_so(booking_reference_number):
	customer = ""
	details = frappe.db.sql(""" select customer from`tabSales Order` where booking_reference_number = %s and docstatus=1""", 					booking_reference_number, as_dict=1)
	if len(details)!=0:
		if details[0]['customer'] is not None:
...
def get_opening_balance(filters, items):
	...
	return frappe.db.sql("""
		select
			...
			sle.company, sle.voucher_type, sle.voucher_no, sle.qty_after_transaction, sle.stock_value_difference
		from
